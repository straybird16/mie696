...
def LoadDatasetByName(DATASET, *args):
    ...
    if DATASET == 'network_data':
        ...
        raw_data = np.loadtxt('../data_backup/ECU_IoHT_regular_data.csv', skiprows=1, delimiter=',', dtype=str)[:,:-2]
        anomalous_raw = np.loadtxt('../data_backup/ECU_IoHT_attack_data.csv', skiprows=1, delimiter=',', dtype=str)[:,:-2]
        features = np.loadtxt('../data_backup/ECU_IoHT_attack_data.csv', max_rows=1, delimiter=',', dtype=str)
        categorical_data_index = [1,2,3]
    # return loaded data
